...
test('defaults each controller to its own robot with fabrics home joints', () => {
  ...
  assert.equal(settings.arms.left.robotName, 'robot_big');
  assert.equal(settings.arms.right.robotName, 'robot_small');
  assert.equal(settings.arms.right.targetPoseTopic, '/robot_small/teleop_command');
...
test('replaces the unconsumed v1 default topic and invalid stored values', () => {
  const settings = parsePanelSettings({
    ...
    arms: {
      ...
      right: { robotName: 'robot_small', targetPoseTopic: 'not-absolute', motion: {} },
    },
